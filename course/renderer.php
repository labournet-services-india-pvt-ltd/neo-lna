...
class core_course_renderer extends plugin_renderer_base {
    ...
    public function course_section_cm_name_title(cm_info $mod, $displayoptions = array()) {
        ...
        $activitylink = html_writer::empty_tag('img', array('src' => $mod->get_icon_url(),
                'class' => 'iconlarge activityicon onlnothers', 'alt' => '', 'role' => 'presentation', 'aria-hidden' => 'true')) .
                html_writer::tag('span', $instancename . $altname, array('class' => 'instancename'));

        //Mihir for LNTOPICS course format
        global $COURSE, $CFG;
        $csformat = course_get_format($COURSE)->get_format();
        if ($csformat == 'lntopics') {
          global $DB;
          $modiconurl = $mod->get_icon_url();
          $thismodtimeline = $DB->get_record('edutimeline', array('cmid' => $mod->id));

          //if (!empty($thismodtimeline)) {
          //  $linetype = $thismodtimeline->type;
            //find the position of colon
            $pos = strpos($instancename,' ',0);
            if ($pos === false) {

            } else {
                $initial = substr($instancename,0,$pos);
                $remaining = substr($instancename,$pos);

                $convertlower = strtolower($initial);
                $instancename = '<b>'.$initial.'</b>'.$remaining;
                
                if ($convertlower == 'video' OR $convertlower == 'lesson' OR $convertlower == 'reading' OR $convertlower == 'activity') {
                  $modiconurl = $CFG->dirrrot.'/course/format/lntopics/pix/'.$convertlower.'.png';
                }
            }

            // if ($linetype == 'VIDEO') {
            //   //$instancename = '<b>'.$linetype.'</b>'.' '.$instancename;
            //   $modiconurl = $CFG->dirrrot.'/course/format/lntopics/pix/video.png';
            // }

          //}

          $activitylink = html_writer::empty_tag('img', array('src' => $modiconurl,
                  'class' => 'iconlarge activityicon onlntopics6', 'alt' => '', 'role' => 'presentation', 'aria-hidden' => 'true')) .
                  html_writer::tag('span', $instancename . $altname, array('class' => 'instancename'));
        }

        //Mihir for LNTOPICS course format END
    ...
    public function course_section_cm_unavailable_error_message(cm_info $cm) {
        ...
        $name = html_writer::empty_tag('img', array('src' => $cm->get_icon_url(),
                'class' => 'iconlarge activityicon okln5', 'alt' => ' ', 'role' => 'presentation')) .
