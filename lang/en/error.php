...
$string['backupcontainexternal'] = 'This backup file contains external Site Network Hosts that are not configured locally';
...
$string['cannotdownloadlanguageupdatelist'] = 'Cannot download list of language updates ';
...
$string['mimetexisnotexist'] = 'Your system is not configured to run mimeTeX. You need to obtain the C source from <a href="https://www.forkosh.com/mimetex.zip">https://www.forkosh.com/mimetex.zip</a>, compile it and put the executable into your Site/filter/tex/ directory.';
...
$string['mssqlrcsmodemissing'] = 'The database is not using the expected READ_COMMITTED_SNAPSHOT mode which can lead to wrong results, especially under high concurrency scenarios. Please enable it for correct behaviour. For further information, see the documentation Installing MSSQL for PHP</a>.';
...
$string['pluginrequirementsnotmet'] = 'Plugin "{$a->pluginname}" ({$a->pluginversion}) could not be installed.  It requires a newer version of Site (currently you are using {$a->currentmoodle}, you need {$a->requiremoodle}).';
...
$string['unicodeupgradeerror'] = 'Sorry, but your database is not already in Unicode, and this version of Site is not able to migrate your database to Unicode.  Please upgrade to Site 1.7.x first and perform the Unicode migration from the Admin page.  After that is done you should be able to migrate to Site {$a}';
...
$string['upgraderequires19'] = 'Error: New Site version was installed on server, unfortunately upgrade from the previous version is not supported.<br />Please upgrade first to latest 1.9.x release. You can also return to previous version by reinstalling original files.';
