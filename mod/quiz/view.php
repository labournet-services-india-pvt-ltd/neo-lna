...
//new code is added by prashant on jan-2020
//5 is for student role
//$x = user_has_role_assignment($USER->id,5);
//if($course->shortname == 'PRE_ASSESSMENT' and $x){
//if course shortname is PRE_ASSESSMENT and login user is student its redirected to /local/assessment/pre_assessmeent_report
if($course->shortname == 'PRE_ASSESSMENT' && !$canpreview){
    redirect(new moodle_url($CFG->wwwroot.'/local/assessment_test/pre_assessment_report.php',array('qid'=>$quiz->id,'userid'=>$USER->id,'cmid'=>$cm->id)));
}
