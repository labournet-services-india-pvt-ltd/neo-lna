...
function bootstrapelements_standard($name, $title, $content, $icon) {
    ...
    //Mihir
    $pos = strpos($title,'Time',0);
    if ($pos === false) {
      // Time word is not present Mihir
      $output .= html_writer::tag('h4', '<i class="fa '.$icon.'"></i>'.$title);
    } else {
        //$title = substr($title,0,$pos);
        $size = 5;
        $title1 = substr($title,5);
        $icon = 'fa-clock-o fa-6 mr-1';
        $output .= html_writer::tag('h4', '<i class="fa '.$icon.'"></i>'.$title1);
    }
...
function bootstrapelements_toggle_outline($togglename, $toggletitle, $togglecontent, $icon) {
    ...
    //Mihir change LN topics

    // $output .= html_writer::tag('a', '<i class="fa '.$icon.'"></i>'.'<span class="togglelntopics">'.$toggletitle.'</span>', array(
    //     'data-toggle' => 'collapse',
    //     'class' => 'accordion-toggle collapsed',
    //     'href' => '#'.$togglename
    // ));

    $output .= html_writer::tag('a', '<span class="togglelntopics">'.$toggletitle.'</span>', array(
        'data-toggle' => 'collapse',
        'class' => 'accordion-toggle collapsed',
        'href' => '#'.$togglename
    ));
