...
-List of Site users|/admin/user.php<br />
...
$string['setting_customnodesusageparameterlinkdd'] = 'The link target can be defined by a full web URL (e.g. https://Site.org) or a relative path within your Site instance (e.g. /login/logout.php).';
...
$string['setting_customnodesusageparametericondd'] = 'This icon will be used as icon for the custom node, for example fa-flag. Use a Font Awesome icon identifier (<a href="https://fontawesome.com/v4.7.0/icons/">See the icon list on fontawesome.com</a>) to identify which icon should be used. Font Awesome is included in Boost, classic Site pix icons are not supported here. If you just want to use a standard bullet icon for the custom node, then leave this field empty. Custom nodes which don\'t get a standard bullet icon automatically also can\'t get a custom icon currently.';
$string['setting_customnodesusageparameteriddt'] = 'ID attribute (optional):';
$string['setting_customnodesusageparameteriddd'] = 'This setting can be used for setting the ID attribute of the custom node to the given fixed value. The given ID value is cleaned for alphanumeric characters only and prefixed by the particular custom node area, for example setting the ID of a node in the bottom nodes for admins area to \'my_node\' will result in an ID attribute filled with \'localboostnavigationcustombottomadminsmynode\'. This setting gives you the ability to reference a custom node in CSS selectors or even in custom JavaScript code. Setting the same ID to multiple nodes is not forbidden here, but please note that duplicate ID attributes are discouraged in HTML pages. If this field is empty, the custom node will get an automatically generated ID which is not really suitable for referencing.';
$string['setting_customnodesusageparameterbeforenodedt'] = 'Before node key (optional):';
$string['setting_customnodesusageparameterbeforenodedd'] = 'This setting can be used to specify the node key above which the custom node should be placed. You find the node key in the \'data-key\' HTML attribute of the node you want to reference here. In case of a set of nodes made of a parent and one or more children, this setting must be given to each node of the set.<br/>Please note that you can only use before node keys from the node context you are adding the node to. For example, you can use \'privatefiles\' or \'calendar\' in the root node context or you can use \'participants\' or \'grades\' in the course node context. If the before node key is unknown or cannot be processed for some unknown reason, the node will be added to the end of the node context.<br />Please also note that this setting is considered partly experimental as is might still not be possible to use some nodes as before nodes with this setting.';
$string['setting_customnodesusagepleasenote'] = 'Please note:';
$string['setting_customnodesusagepleasenotecss'] = 'Due to the way how Boost\'s nav drawer is built in Site core, all custom nodes are displayed equally. Adding custom CSS classes, custom HTML element ids or a target attribute to open the link in a new window is impossible.';
$string['setting_customnodesusagepleasenotecheck'] = 'If the custom node does not show up in Boost\'s nav drawer, please check if all mandatory params are set correctly, if the optional language setting fits to your current Site user language and if your other optional parameters apply to your user.';
...
$string['setting_customnodesusageusersexample'] = 'Site.org website|http://www.Site.org|en,de<br />
...
$string['setting_removebadgescoursenodeexplanation'] = 'The node is only removed if there are no badges in the course. Teachers can still always access the badges management page in the course menu (cog menu).<br /><em>This setting is only processed when the badges subsystem is enabled in Site.</em>';
$string['setting_removecompetenciescoursenodeexplanation'] = 'The node is only removed if there are no competencies in the course. For teachers, there will be another "Competencies" node added to the course menu (cog menu).<br /><em>This setting is only processed when the competencies subsystem is enabled in Site.</em>';
$string['setting_removecoursenodestechnicalhint'] = 'Technical background: This is done by removing the node from the navigation tree. Thus, the node cannot be accessed by other parts of Site anymore. In normal Site setups, this should hopefully not cause any trouble.';
...
$string['setting_removerootnodestechnicalhint'] = 'Technical background: This is done by setting the node\'s showinflatnavigation attribute to false. Thus, the node will only be hidden from the nav drawer, but it will remain in the navigation tree and can still be accessed by other parts of Site.';
